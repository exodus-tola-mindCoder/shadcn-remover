...
async function checkComponentsDir(): Promise<boolean> {
  ...
  if (!exists) {
    console.error(
      chalk.red(`❌ Error: Directory not found: ${COMPONENTS_DIR}`)
    );
    console.log(
      chalk.yellow(
        "Please ensure you are running this command from the root of your project and the path is correct."
      )
    );
    process.exit(1);
  }
...
async function getAllComponents(): Promise<string[]> {
  ...
  try {
    ...
    console.error(
      chalk.red(`❌ Error reading components directory: ${COMPONENTS_DIR}`)
    );
...
async function removeComponent(
  ...
  if (removedPath) {
    ...
    if (dryRun) {
      console.log(
        chalk.yellow(`[Dry Run] Would remove ${removedType}: ${relativePath}`)
      );
    } else {
      try {
        ...
        console.error(
          chalk.red(`❌ Failed to remove ${removedType}: ${componentName}`)
        );
        if (error instanceof Error) {
          console.error(error.message);
        } else {
    ...
    console.log(
      chalk.yellow(
        `❓ Component "${componentName}" not found (checked for ${componentName}.tsx and directory ${componentName})`
      )
    );
...
async function getPackageVersion(): Promise<string> {
  ...
  try {
    ...
    console.warn(
      chalk.yellow(`⚠️ Could not read version from ${packageJsonPath}.`)
    );
...
async function removeImportsFromFile(
  filePath: string,
  ...
  if (newContent !== content) {
    ...
    if (dryRun) {
      console.log(
        chalk.yellow(`[Dry Run] Would remove imports from: ${relativePath}`)
      );
    } else {
...
(async () => {
  try {
    ...
    program
      ...
      .option(
        "-a, --all",
        "Attempt to remove all detected Shadcn UI components",
        false
      )
      .argument(
        "[components...]",
        "Specific component names to remove (e.g., button card dialog)"
      )
      .action(async (components: string[], options: CLIOptions) => {
        ...
        if (removeAll) {
          if (components.length > 0) {
            console.log(
              chalk.yellow(
                "Warning: Specific components provided along with --all flag. Ignoring specific components and removing all."
              )
            );
          }
          ...
          if (componentsToDelete.length === 0) {
            console.log(
              chalk.yellow("🟡 No components found in components/ui directory.")
            );
            process.exit(0);
          }
          console.log(
            chalk.blue(
              `ℹ️ Attempting to remove all ${componentsToDelete.length} detected components.`
            )
          );
          ...
          if (allComponents.length === 0) {
            console.log(
              chalk.yellow(
                "🟡 No components found to select from in components/ui directory."
              )
            );
            process.exit(0);
          }
        ...
        if (componentsToDelete.length === 0) {
          console.log(
            chalk.yellow(
              "🟡 No components selected or specified for removal. Exiting."
            )
          );
          process.exit(0);
        }
        ...
        // Confirmation step
        console.log(
          chalk.cyan(`Selected components: ${componentsToDelete.join(", ")}`)
        );
        ...
        // Execution step
        const spinner = ora(
          dryRun ? "Simulating component removal..." : "Removing components..."
        ).start();
        ...
        const results = await Promise.allSettled(
          componentsToDelete.map((component) =>
            removeComponent(component, dryRun)
          )
        );
        ...
        if (failCount > 0) {
          spinner.warn(
            chalk.yellow(
              `Completed with ${failCount} error(s). ${successCount} component(s) processed.`
            )
          );
        } else if (dryRun) {
          spinner.succeed(
            chalk.green(
              `Dry run complete. ${successCount} component(s) simulated for removal.`
            )
          );
        } else {
          spinner.succeed(
            chalk.green(`Successfully removed ${successCount} component(s).`)
          );
        }
